...
export default function SaaSContractAnalyzer({
  backgroundImageUrl = "/grey-brick-wall.png",
  overlayOpacity = 0.4,
  ...
  > = {
    ...
    // Communication - Updated 2025 pricing
    Discord: {
    ...
    "Google Meet": {
      ...
      avgPricePerUser: 6, // Part of Google Workspace Business Starter
      marketPosition: "Budget",
    },
    "Microsoft Teams": {
      category: "Communication",
      features: ["Video Conferencing", "Team Chat", "File Sharing"],
      avgPricePerUser: 6, // Business Basic pricing
      marketPosition: "Standard",
    },
    Slack: {
      category: "Communication",
      features: ["Team Messaging", "File Sharing", "App Integration"],
      avgPricePerUser: 8.75, // Pro plan monthly pricing
      marketPosition: "Standard",
    },
    Zoom: {
      category: "Communication",
      features: ["Video Conferencing", "Screen Sharing", "Recording"],
      avgPricePerUser: 15, // Pro plan pricing
      marketPosition: "Standard",
    },

    // CRM - Updated 2025 pricing
    Freshsales: {
      category: "CRM",
    ...
    HubSpot: {
      ...
      avgPricePerUser: 20, // Starter plan
      marketPosition: "Standard",
    },
    Pipedrive: {
      category: "CRM",
    ...
    Salesforce: {
      ...
      avgPricePerUser: 80, // Pro Suite pricing
      marketPosition: "Premium",
    },
    "Zoho CRM": {
      category: "CRM",
    ...
    // Observability & Monitoring - Updated with 2025 accurate pricing
    Dynatrace: {
      category: "Observability & Monitoring",
      features: ["Full-stack Monitoring", "AI-powered Analytics", "Real-time Insights"],
      avgPricePerUser: 74, // Updated: $74 per 8GB host for full-stack monitoring
      marketPosition: "Premium",
    },
    "New Relic": {
      category: "Observability & Monitoring",
      features: ["APM", "Infrastructure", "Logs & Metrics"],
      avgPricePerUser: 99, // Updated: $99 per full platform user (Standard edition)
      marketPosition: "Standard",
    },
    AppDynamics: {
      category: "Observability & Monitoring",
    ...
    Datadog: {
      ...
      avgPricePerUser: 46, // Updated: $31 APM + $15 Infrastructure (combined pricing)
      marketPosition: "Standard",
    },
  }
  ...
  // ---------- Enhanced discount modeling helpers with 2025 market insights ----------
  ...
  function sizeBandBySeats(seats: number) {
    // Updated discount bands based on 2025 market research
    if (seats >= 5000) return { range: [25, 35] as [number, number] } // Enterprise/Global
    if (seats >= 500) return { range: [15, 25] as [number, number] } // Large enterprise
    if (seats >= 50) return { range: [10, 15] as [number, number] } // Mid-market
    if (seats >= 1) return { range: [5, 10] as [number, number] } // Small business
    return { range: [0, 0] as [number, number] }
  }

  function categoryAdjustments(category: string) {
    let adj = 0
    // Infrastructure/platform tools often have higher discounts due to competitive pressure
    if (["Observability & Monitoring", "Development Tools"].includes(category)) adj += 5
    // High-growth categories with lots of competition
    if (["Communication", "Project Management"].includes(category)) adj += 3
    // Specialized/niche typically lower flexibility
    if (["AI & ML", "Security & IT", "Design & Creative"].includes(category)) adj -= 3
    // Enterprise categories with complex sales cycles
    if (["Finance & Accounting", "HR & Recruiting"].includes(category)) adj += 2
    return adj
  }

  function positionAdjustments(position: Contract["marketPosition"]) {
    if (position === "Premium") return -3 // Premium vendors have less discount flexibility
    if (position === "Budget") return +2 // Budget vendors often discount to win deals
    return 0
  }

  function contractLengthAdjustments(length: number) {
    // Multi-year commitments significantly increase discount potential
    if (length >= 36) return [15, 20] as [number, number] // 3+ year deals
    if (length >= 24) return [10, 15] as [number, number] // 2+ year deals
    if (length >= 12) return [5, 10] as [number, number] // 1+ year deals
  ...
  function expectedFor(contract: Contract) {
    ...
    const lengthAdjustment = contractLengthAdjustments(contract.contractLength)
    range = addToRange(range, lengthAdjustment)

    // Volume/scale adjustments
    if (seats >= 1000)
      range = addToRange(range, 12) // Very large deals
    else if (seats >= 500)
      range = addToRange(range, 8) // Large deals
    else if (seats >= 100) range = addToRange(range, 5) // Medium deals
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {!showResults ? (
            ...
            <div className="space-y-6">
              <div className="bg-gradient-to-r from-green-500 to-green-600 text-white rounded-2xl p-8 shadow-xl">
                <div className="text-center">
                  ...
                  <div className="text-sm opacity-75 mt-4">
                    💡 Based on 2025 market rates and realistic enterprise discount bands
                  </div>
                </div>
              </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200">
                        ...
                        <th className="text-left py-3 px-4 font-medium text-gray-900">Your Price/Unit/Month</th>
                        ...
                        <th className="text-left py-3 px-4 font-medium text-gray-900">Target Price Range</th>
                          ...
                          <tr key={contract.id} className="border-b border-gray-100">
                            <td className="py-4 px-4">
                              <div>
                                ...
                                <div className="text-sm text-gray-500">
                                  {volume} {contract.unitLabel || "units"}
                                </div>
                                <div className="text-xs text-gray-400">{contract.contractLength} month contract</div>
                              </div>
                            </td>
                            <td className="py-4 px-4">
                              <div className="font-medium">${yourPrice.toFixed(0)}</div>
                              <div className="text-xs text-gray-500">per {contract.unitLabel || "unit"} / mo</div>
                            </td>
                            <td className="py-4 px-4">
                            ...
                            <td className="py-4 px-4">
                              ...
                              <div className="text-xs text-gray-500 mt-1">
                                {volume >= 500 ? "Large scale" : volume >= 100 ? "Medium scale" : "Small scale"}
                              </div>
                            </td>
                            <td className="py-4 px-4">
                              ...
                              <div className="text-xs text-gray-500">
                                target: ${Math.round(targetMid).toLocaleString()}
                              </div>
                            </td>
                            <td className="py-4 px-4">
                              {status === "Unknown" ? (
              ...
              {overlaps.length > 0 && (
                <div className="bg-white rounded-2xl shadow-xl p-8">
                  <h2 className="text-2xl font-semibold mb-6">Feature Overlaps</h2>
                  <div className="space-y-4">
                    {overlaps.map((overlap, index) => (
                      <div key={index} className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div className="font-medium text-yellow-800 mb-2">
                          {overlap.vendor1} ↔ {overlap.vendor2}
                        </div>
              ...
              <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-2xl p-8 text-center">
                ...
                <p className="text-xl mb-6 opacity-90">
                  Our expert negotiators have saved companies like yours millions on SaaS renewals using 2025 market
                  data.
                </p>
                ...
                <div className="mt-6 text-sm opacity-75">
                  <p>✓ Updated 2025 market rates ✓ Enterprise discount modeling ✓ Average client saves 18%</p>
                </div>
              </div>
